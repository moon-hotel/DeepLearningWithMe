"""
文件名: Code/Chapter03/main.py
创建时间: 2023/2/6 8:04 下午
"""
...
def prediction(x_test, y_test, x_train, y_train):
    ...
    plt.rcParams['font.sans-serif'] = ['Arial Unicode MS']
    plt.scatter(x_train, y_train, c='black', label='训练数据')
    plt.plot(x_test, y_pre_1, linestyle='--', label=r'Model A, R^2 = {}'.format(round(r2_1, 2)), c='black')
    plt.plot(x_test, y_pre_5, label=r'Model B, R^2 = {}'.format(round(r2_5, 2)), c='black')
    plt.plot(x_test, y_pre_10, linestyle='dashdot', label=r'Model C, R^2 = {}'.format(round(r2_10, 2)), c='black')
    plt.tight_layout()  # 调整子图间距
    # plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定默认字体
    plt.legend(fontsize=13)
    plt.show()
...
def train(x_train, y_train):
    ...
    plt.plot(x_train, y_pre_1, linestyle='--', label=r'Model A, R^2 = {}'.format(round(score_1, 2)), c='black')
    plt.plot(x_train, y_pre_5, label=r'Model B, R^2 = {}'.format(round(score_5, 2)), c='black')
    plt.plot(x_train, y_pre_10, linestyle='dashdot',
             label='Model C, R^2 = {}'.format(round(score_10, 2)), c='black')
    plt.tight_layout()  # 调整子图间距
    # plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定默认字体
    plt.legend(fontsize=13)
    plt.show()
