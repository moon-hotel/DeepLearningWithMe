...
class ModelConfig(object):
    def __init__(self,
        ...
        self.model_save_path = 'lenet6.pt'
        self.summary_writer_dir = "runs/lenet6"
        self.device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')
        logger_init(log_file_name='lenet6', log_level=logging.INFO, log_dir='log')
...
def train(config):
    ...
    # model = LeNet6()  # 前3层随机初始化
    model = LeNet6.from_pretrained("../C04_ModelSaving", freeze=False)  # 前3层用LeNet5中的前3层初始化
    ...
    for epoch in range(config.epochs):
        ...
        if test_acc > max_test_acc:
            ...
            logging.info(f" # 保存模型{config.model_save_path}成功……")
    return model
