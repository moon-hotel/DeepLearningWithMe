...
5         logits = net(x)
6         acc_sum += (logits.argmax(1) == y).float().sum().item()
7         n += len(y)
8     return round(acc_sum / n, 4)
```

在上述代码中，第2行为将模型的状态设定为推理转态，因为在推理阶段不需要进行Dropout操作，同时这一步PyTorch内部已经帮我们实现，不需要自己写逻辑判断是否要进行Dropout；第4~8行是对迭代器中的所有样本进行预测，然后记录预测正确的数量和总数量，并返回最后的准确率。
